...
def process_single_frame_threshold(frame_idx, raw_arr, params):
    ...
    sigma_value, thresh_adj_mult, min_obj_size, overlap_thresh, transition_thresh, window_size, averaging_threshold = params
...
def process_single_frame_instance_separation(frame_idx, raw_arr, instance_arr, averaged_binary_masks, params):
    ...
    transition_labels = np.zeros_like(frame_objects, dtype=np.int32)
    ...
    if np.any(frame_objects):
        ...
        for region in regionprops(frame_objects):
            ...
            if transition_thresh > overlap_ratio >= overlap_thresh: #0.95 overlap ratop 0.05
                # Mark as transition perinuclear
                for y, x in coords:
                    transition_labels[y, x] = region.label
            elif overlap_ratio >= transition_thresh: #overlap ratio >= 0.95
...
def process_perinuclear_mask(raw_arr, instance_arr, output_path, sigma_value=10, thresh_adj_mult=1.3, min_obj_size=5000, overlap_thresh=0.05, transition_thresh=0.90, window_size=10, averaging_threshold=0.5, max_workers=None):
    # Process in parallel
    ...
    transition_filename = output_path.replace('.npz','_transition_mask.npz')
    ...
    transition_full = np.zeros((num_frames, height, width), dtype=np.int32)
    telenuclear_full = np.zeros((num_frames, height, width), dtype=np.int32)

    params = (sigma_value, thresh_adj_mult, min_obj_size, overlap_thresh, transition_thresh, window_size, averaging_threshold)
...
def process_single_parameter(sample_num, thresh_adj_mult, overlap_thresh, transition_thresh, data_dir, output_root_dir, 
    ...
    param_dir_name = f"Sample{sample_num}_thresh_adj_mult_{thresh_adj_mult:.1f}_tele_overlap_{overlap_thresh:.2f}_peri_overlap_{transition_thresh:.2f}"
